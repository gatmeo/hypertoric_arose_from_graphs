...
\usepackage[quaderno,common_math_textnormal,todonotes,math_base_note,math_simple]{gatmeo}
\usepackage{tikz-cd}
...
\missing{Relating Symplectic Reduction and GIT: Frances Kirwan's thesis}
...
\begin{proposition}[pps:polystable_hypertoric]{}
...
\begin{proposition}[pps:stable_hypertoric]{}
...
\begin{proposition}[pps:]{}
  ...
  \begin{proof}
    ...
    \]
    is coindependent.
    %We want to alter the representation of $\chi$ such that $a_i>0$ for $z_i\neq 0$ and $b_i>0$ for $w_i\neq 0$. 
    ...
    \]
    as $\iota^*\circ \pi^*=0$. By \Cref{pps:polystable_hypertoric} and \Cref{pps:stable_hypertoric}, $(z,w)\in \mu^{-1}(\lambda)^{\chi-\stab}$.
  \end{proof}
\end{proposition}
...
\begin{proposition}[pps:ktheory_hypertoric]{}
...
\[
  ...
  \end{cases}
\]
By construction, $p_{B,\lambda}$ is $\chi$--semistable as one can define the homogenous function on it by 
\[
f=\prod_{i\in B^+}^{}z_i^{\rho_B^*(\chi)_e}\prod_{i\in B^-}^{}w_i^{\rho_B^*(\chi)_e}.
\]
...
\begin{corollary}[crl:tautological_bundle_tchbar_weight]{}
  ...
  \]
  \begin{proof}
      It follows from \Cref{pps:ktheory_hypertoric} and the previous corollary.
  \end{proof}
\end{corollary}
...
\begin{theorem}[thm:cohomology_hypertoric]{\cite{konno2001equivariant}}
...
Here we cite the version of Excess Intersection formula for the case where all subvarieties are smooth:
 ...
 %a Cartier divisor and $\iota:X\rightarrow M$ the inclusion of a subvariety $X$ in $M$. If 
...
\begin{theorem}[thm:stable_envelope]{\cite{maulik2012quantum}}
...
\begin{proposition}[pps:]{}
    ...
    \begin{proof}
        ...
        \]
        Note that $\mathcal{N}_{X_{B^*_+,B^*_-}}|_{p_B}=\mathcal{N}_{\overline{\Attr(p_B)}}|_{p_B}=\mathcal{N}^-|_{p_B}$ by \Cref{pps:attr_nnegative}.
        On the other hand,
        by \Cref{crl:tautological_bundle_tchbar_weight}, the $T$-weights of $\NN^+_{p_B}$ and $\NN^{1/2}_{p_B}$ are $\CB{\fundcyclewithtree{i}}_{i\in B^*_+}\cup \CB{-\fundcyclewithtree{i}}_{i\in B^*_-}$ and $\CB{\fundcyclewithtree{i}}_{i\in B^*}$ respectively.
        The proof then follows from (2) in \Cref{thm:stable_envelope}.
    \end{proof}
\end{proposition}


\missing{Comparing Stability Condition with Quiver Varieties}
\appendix


\bibliographystyle{alpha}
\bibliography{ref_hypertoric_arose_from_graphs}
