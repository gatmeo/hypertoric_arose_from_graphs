...
\begin{proposition}[pps:tautological_line_bundle_T_weight]{}
...
\begin{remark}
    Here, 
    $u_i=c_1(L_i)=[\CB{z_i=0}]$, $\hbar=c_1(\underline{\mathbb{C}}_\hbar)$ are of degree 2.
\end{remark}
\begin{remark}
...
\]
The cohomology class for such $X_\varepsilon$ has the following nice description:
\begin{proposition}[pps:]{}
    $\hbar^k[X_{\varepsilon}]=\prod_{\varepsilon(i)=-}^{}u_i\prod_{\varepsilon(i)=+}^{}(\hbar-u_i)$, where $k=\dim G$.
    \begin{proof}
        By excess intersection formula,
        \[
            \prod_{\varepsilon(i)=-}^{}u_i\prod_{\varepsilon(i)=+}^{}(\hbar-u_i)=(\iota_{X_{\varepsilon}})_*\left(\left[c(N_{X_\varepsilon})^{-1}\prod_{\varepsilon(i)=-}^{}c(L_i|_{X_\varepsilon})\prod_{\varepsilon(i)=+}^{}c(L_i^*|_{X_\varepsilon}\otimes\underline{\mathbb{C}}_\hbar)\right]\right).
        \]
    \end{proof}
    \missing{}
\end{proposition}
...
Define the \vocab{full attracting cell} of $Z$ by $\fAttr(Z):=\bigsqcup_{Z'\leq Z}\Attr(Z')$.

One can define two ways to decomposte $TX$. The first one uses $\sigma$ to define a sign on the tangent weights. Since we have the short exact sequence $0\rightarrow TZ\rightarrow TX|_Z\rightarrow N_Z\rightarrow 0$, and the $T$ weights on $TZ$ is $0$, we have the following
\[
    N^+_:=\bigoplus _{\AB{\sigma,\gamma}>0}\left(N_Z\right)_{\gamma}=\bigoplus _{\AB{\sigma,\gamma}>0}\left(TX|_{Z}\right)_{\gamma}
    \textnormal{ and }
    N^-:=\bigoplus _{\AB{\sigma,\gamma}<0}\left(N_Z\right)_{\gamma}=\bigoplus _{\AB{\sigma,\gamma}<0}\left(TX|_{Z}\right)_{\gamma}.
\]
\begin{proposition}[pps:]{}
    For $Z\in \pi_0(X^T)$, $N^-_Z\simeq N^+_Z\otimes\underline{\mathbb{C}}_\hbar$ and $N_{\overline{\Attr(Z)}}|_{Z}=N^-_Z$.
    \begin{proof}
...
The second way to descompose $TX$ is called \vocab{polarization} as an auxiliary choice. A \vocab{local polarization} at $Z$ is a choice of $\varepsilon_Z\in \HH^*_T(Z)$ such that $\varepsilon_Z^2=(-1)^{\frac{1}{2}\codim Z}e^T(N_Z)$. A \vocab{gloabl polarization} is a choice of $T^{1/2}\in K_{T\times \Chbar}$ such that $[TX]=T^{1/2}+\hbar(T^{1/2})^*$.
Given a global polarization $T^{1/2}$, one can define $\varepsilon_Z:=e^T(N_{Z}^{1/2})$, where $N_Z^{1/2}:=[N_Z]\cap T^{1/2}|_{Z}$.

A \vocab{stable envelope}, $\Stab:\HH^*_{T\times \Chbar}(X^T)\rightarrow \HH^*_{T\times \Chbar}(X)$, is a map of $\HH^*_{T\times \Chbar}$-modules that satisfies the following properties, stated as a theorem:
...
\begin{theorem}[thm:stable_envelope]{}
    ...
    \begin{enumerate}
        ...
        \item $\Stab(\alpha)|_Z=\pm e^{T\times \Chbar}(N_Z^-)\alpha$, where $\pm$ satisfies $\varepsilon_Z=\pm e^T(N_Z^-)$,
    ...
    \end{enumerate}
    \begin{remark}
    Given $T^{1/2}$, i.e. $\varepsilon_Z$, Since both $N_Z^{1/2}$ and $N_Z^-$ span half of $N_Z$, we have $r=\rank([N_Z^+]\cap N_Z^{1/2})=\rank([N_Z^-]\cap (N_Z^{1/2})^*)$. Hence one can compute $\pm$ in (2) as $(-1)^r$.
    \end{remark}
\end{theorem}

\begin{theorem}[thm:]{}
    $\Stab$ becomes an isomorphsim after inverting the $T$-weights in $N_{X^T}$.
\end{theorem}
...
\begin{proposition}[pps:]{}
    ...
    \begin{proof}
        By \Cref{pps:tautological_line_bundle_T_weight}, the $T$-weights of $N^+_{p_B}$ and $N^{1/2}_{p_B}$ are $\CB{\fundcyclewithtree{i}}_{i\in B^*_+}\cup \CB{-\fundcyclewithtree{i}}_{i\in B^*_-}$ and $\CB{\fundcyclewithtree{i}}_{i\in B^*}$ respectively.
        The proof then follows from (2) in \Cref{thm:stable_envelope} and
        \missing{$e^{T\times \Chbar}(N_Z^-)=[X_{B^*_+,B^*_-}]$}
    \end{proof}
\end{proposition}
